...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=PROJECT_ROOT / "config/region_descriptions.json",
        help="Path to region_descriptions.json (default: %(default)s).",
    )
    parser.add_argument(
...
def is_left_camera_reference(spatial_reference: Any, camera_perspective: Any) -> bool:
    return (
        ...
        and str(camera_perspective or "").strip().lower() == "side"
    )
...
def remap_states_for_reference(
    ...
    *,
    ...
    Mapping selection:
      - (camera_perspective='side', spatial_reference='camera') -> camera2leftCamera_reference
