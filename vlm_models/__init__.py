...
This trimmed package only ships the ``gpt_schema`` backend. To add another
model, drop a new module under ``vlm_models/`` that defines a ``BaseVLM``
subclass decorated with ``@register`` and import it below.
"""
...
# Import the shipped backend so its ``@register`` decorator runs.
